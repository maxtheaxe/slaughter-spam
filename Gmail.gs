...
function createDeleteFilter (fromAddress) {
	...
	// wanted to add setting to allow usage of spam tag instead of trash,
	// but google doesn't allow that for some reason (probably because
	// they call that "blocking" instead, and it doesn't work like a
	// normal filter)
...
/**
 * all together now: deletes target thread and all future mail from that sender
 */
